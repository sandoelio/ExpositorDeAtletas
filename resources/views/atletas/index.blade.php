...
<style>
    ...
    .flip-back .dados p { margin: 2px 0; }
    .badge-pos, .badge-back {
        background: #e66000;
        color: #fff;
        padding: 4px 8px;
        border-radius: 6px;
    ...
    @media (max-width: 768px) {
        ...
        .flip-front .foto-front { flex: 0 0 50%; }
        .flip-front .info { flex: 0 0 50%; padding: 12px; }
        .flip-front .info h3 { font-size: 14px; }
        .flip-front .posicao { font-size: 30px; }
        .flip-back .foto-back { flex: 0 0 80px; }
        .flip-back .foto-back img { width: 100px; height: 150px; }
        .flip-back .dados { font-size: 15px; }
    }

    .filtros-row { margin-bottom: 18px; }
</style>
...
<div class="container">
            ...
            <select id="cidade" class="form-control">
                ...
                @if(isset($cidades) && count($cidades))
                    @foreach($cidades as $c)
                        <option value="{{ is_object($c) ? $c->cidade : $c }}">{{ is_object($c) ? $c->cidade : $c }}</option>
                    @endforeach
                @endif
            </select>
        </div>
            ...
            <select id="entidade" class="form-control">
                ...
                @if(isset($entidades) && count($entidades))
                    @foreach($entidades as $e)
                        <option value="{{ is_object($e) ? $e->entidade : $e }}">{{ is_object($e) ? $e->entidade : $e }}</option>
                    @endforeach
                @endif
            </select>
        </div>
        <div class="col-12 mt-2 text-end">
            <button type="button" class="btn" style="background:#e66000;color:#fff" onclick="buscarAtletas()">Filtrar</button>
    ...
    <div class="row g-3" id="lista-atletas">
        @forelse($atletas as $atleta)
            <div class="col-md-4 text-center atleta-card" data-idade="{{ $atleta->idade ?? '' }}"
                    ...
                    <div class="flip-card-inner">
                        <div class="flip-front">
                            <div class="foto-front">
                                <img src="{{ !empty($atleta->imagem_base64) ? 'data:image/png;base64,'.$atleta->imagem_base64 : asset('img/avatar.png') }}" alt="Foto de {{ $atleta->nome_completo }}">
                            </div>
                            <div class="info">
                                ...
                                <div class="viz-counter-wrapper badge-pos" style="margin-top:6px;">
                                    👁️ <span class="viz-counter" id="visualizacoes-{{ $atleta->id }}">{{ (int) ($atleta->visualizacoes ?? 0) }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="flip-back">
                            <div class="conteudo">
                                <div class="foto-back">
                                    <img src="{{ asset('img/basket-silhouette.png') }}" alt="Foto de {{ $atleta->nome_completo }}">
                                </div>
                                <div class="dados">
